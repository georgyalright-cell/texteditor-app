(function attach(root, factory) {
  ...
  function safeMessage(value) {
    return String(value || "").replace(/https?:\/\/[^\s<>"']+/giu, value => {
      try { const url = new URL(value); return `${url.origin}${url.pathname}`; } catch { return "[URL]"; }
    }).replace(/(?:bearer\s+|(?:token|api[_-]?key|password|secret)\s*[:=]\s*)[^\s,;]+/giu, "[скрыто]")
      .replace(/[\u0000-\u001f\u007f]/gu, " ").slice(0, 1200);
  }
  ...
  function event(error, { stage = "Локальная обработка", offline = false } = {}) {
    const info = explain(error, { offline });
    // Only the runtime error, never prompts, document content or request payloads.
    // Keep this in page memory; strip URL credentials/query/fragment and control chars.
    const technical = safeMessage(`${error && error.name || "Error"}: ${String(error && error.message || error || "")}`);
    return { message: info.message, isError: true,
      diagnostic: { category: info.category, message: info.message, stage, technical } };
  }
  ...
  return { explain, event, retryOnce };
});
