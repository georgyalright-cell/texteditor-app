(function attachRuleParaphraser(root, factory) {
  ...
  const ACADEMIC_RU = [
    ...
    // «В результате» отсюда убрано: оно утверждает причинность, которой в
    // исходном «таким образом» нет — тот подводит итог. Сигнал при этом не
    // отмывался (оба слова в DISCOURSE_RU), портился смысл. «Итак» подводит
    // итог так же и тоже под наблюдением.
    [/\bтаким образом,/giu, ["следовательно,", "итак,"], { variation: true }],
  ];
