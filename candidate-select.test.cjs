...
test("вывод модели проходит словарь и типографику до оценки", () => {
  // Прежде модельный вариант судился как есть: со штампом он просто проигрывал
  // и выбрасывался, хотя после одной словарной правки был бы лучшим.
  const source = "Команда собрала прототип за шесть недель. Отчёт ушёл заказчику в тот же день. Замечаний не поступило.";
  const outcome = select.polishSentences(source, {
    language: "ru",
    limit: 1,
    generate: () => Promise.resolve(["В современном мире команда собрала прототип за шесть недель."]),
  });
  return outcome.then((result) => {
    assert.doesNotMatch(result.text, /в современном мире/iu);
  });
});

test("приведение не имеет права разойтись с якорями исходника", () => {
  // Словарь якоря не трогает, но проверять дешевле, чем доверять: если после
  // приведения число или ссылка разошлись, берётся неприведённый вариант.
  const source = "Выручка выросла на 12,5% за 2024 год. Отчёт ушёл заказчику в тот же день. Замечаний не поступило.";
  return select.polishSentences(source, {
    language: "ru",
    limit: 1,
    generate: () => Promise.resolve(["За 2024 год выручка выросла на 12,5%."]),
  }).then((result) => {
