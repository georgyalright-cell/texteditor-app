...
test("метрика повторяет прежние критерии: антитеза поднимает итог", () => {
  ...
  // Одна антитеза в русском — обычная конструкция: она есть у 28% фрагментов
  // корпуса живых текстов. Признаком становится вторая.
  const tic = metrics.scoreText("Это не просто продукт, а решение. Рынок не просто стабилен, а растёт. Спрос высок.");
